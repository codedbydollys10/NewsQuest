...
import { generateCauseChain, generateCauseChainFeedback, validateCauseChain, calculateXPReward, } from '../services/causeChainService.js';
const router = Router();
...
const parseString = (value, defaultValue = '') => typeof value === 'string' ? value.trim() : defaultValue;
...
router.post('/submit', async (req, res) => {
    try {
        ...
        if (nodes.length > 0 && edges.length > 0) {
            validation = validateCauseChain(userConnections, edges, userChain, nodes);
        }
        else {
        ...
        if (nodes.length > 0) {
            aiFeedback = await generateCauseChainFeedback(nodes[0]?.text || 'Challenge', nodes, edges, userChain, userConnections);
        }
