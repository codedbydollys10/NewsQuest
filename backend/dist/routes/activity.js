...
const parseActivityType = (value) => {
    if (value === 'read' || value === 'quiz' || value === 'prediction' || value === 'battle')
        return value;
    return null;
};
...
const parseOptionalStreak = (value) => {
    if (typeof value !== 'number' || !Number.isFinite(value))
        return undefined;
    return Math.max(0, Math.floor(value));
};
const parseOptionalString = (value, maxLength) => {
    if (typeof value !== 'string')
        return undefined;
...
router.post('/track', async (req, res, next) => {
    try {
        ...
        const row = await recordUserActivity(userId, type, req.body?.at, parseOptionalStreak(req.body?.streakCount), {
