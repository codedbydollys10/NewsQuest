...
import { syncUserTables, validateAccessToken } from '../lib/profileSync.js';
const router = Router();
const readString = (value, fallback = '') => (typeof value === 'string' ? value.trim() : fallback);
const readNumber = (value, fallback = 0) => (typeof value === 'number' && Number.isFinite(value) ? value : fallback);
const readStringArray = (value) => (Array.isArray(value) ? value.filter((entry) => typeof entry === 'string') : []);
const readAvatarCustomization = (value) => {
