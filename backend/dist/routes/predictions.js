...
const buildQuery = (headline, summary, category) => {
...
const scoreOptionsAgainstContext = (options, context) => {
...
const scoresToProbabilities = (scores) => {
    if (!scores.length)
        return [];
...
`;
    ...
    try {
        ...
        if (!Number.isInteger(payload.resolvedIndex))
            return null;
        if (payload.resolvedIndex < 0 || payload.resolvedIndex >= input.options.length)
            return null;
        ...
        const probabilities = safeProbabilities
            ? (() => {
                ...
                if (!sum)
                    return scoresToProbabilities(new Array(input.options.length).fill(1));
...
router.post('/resolve', async (req, res, next) => {
    try {
        ...
        let latest;
        try {
            latest = await withTimeout(searchNews(q, {
        ...
        let aiPick = null;
        try {
            aiPick = await withTimeout(tryBytezResolution({
                question: parsed.question,
                options: parsed.options,
                context,
            }), 6000, 'Bytez resolution');
        }
        catch (bytezError) {
