...
const parseRows = async (response) => {
    const rows = await response.json().catch(() => null);
    return Array.isArray(rows) ? rows : [];
};
const restUpsert = async (table, row, onConflict = 'id') => {
...
const replaceUserInterests = async (userId, interests) => {
    const base = getSupabaseBase();
...
const upsertQuizResults = async (profile) => {
    const base = getSupabaseBase();
    ...
    if (patchResponse.ok) {
        const rows = await parseRows(patchResponse);
        if (rows.length > 0)
            return;
    }
...
const upsertUserActivity = async (profile) => {
    const date = new Date(profile.last_active_date).toISOString().slice(0, 10);
    const actionCount = Math.max(0, profile.articles_read + profile.quizzes_total + profile.predictions_total + profile.wins + profile.losses + profile.draws);
    await restUpsert('user_activity', {
        user_id: profile.id,
        activity_date: date,
        action_count: actionCount,
    }, 'user_id,activity_date');
};
const syncArticlesReadSnapshot = async (_profile) => {
    // Keep historical article rows intact; reads are tracked per event in activity tracking.
    return;
};
const getProfileRank = (profile) => {
    if (profile.battle_tier && profile.battle_tier.trim())
        return profile.battle_tier.trim();
    if (profile.current_level >= 20)
        return 'LEGEND';
    if (profile.current_level >= 15)
        return 'MASTER';
    if (profile.current_level >= 10)
        return 'ORACLE';
    if (profile.current_level >= 6)
        return 'STRATEGIST';
    if (profile.current_level >= 3)
        return 'ANALYST';
    return 'ROOKIE';
};
export const syncUserTables = async (profile) => {
