...
const extractNumberClues = (text) => Array.from(new Set((text.match(/\b\d+(?:\.\d+)?%?\b/g) ?? []).map((value) => value.trim()))).slice(0, 6);
const pickTopic = (article) => {
    const sourceText = `${article.headline} ${article.summary} ${article.category}`.toLowerCase();
    if (sourceText.includes('ai') || sourceText.includes('artificial intelligence') || sourceText.includes('neural'))
        return 'AI';
    if (sourceText.includes('election') || sourceText.includes('vote') || sourceText.includes('parliament') || sourceText.includes('court'))
        return 'Politics';
    if (sourceText.includes('market') || sourceText.includes('stock') || sourceText.includes('economy') || sourceText.includes('trade'))
        return 'Economy';
    if (sourceText.includes('space') || sourceText.includes('mars') || sourceText.includes('nasa') || sourceText.includes('isro'))
        return 'Science';
    if (sourceText.includes('health') || sourceText.includes('medical') || sourceText.includes('hospital') || sourceText.includes('drug'))
        return 'Health';
    if (sourceText.includes('sport') || sourceText.includes('cricket') || sourceText.includes('football') || sourceText.includes('olympic'))
        return 'Sports';
...
const seenPredictionTexts = new Set();
const PREDICTION_CACHE_LIMIT = 120;
const normalizeText = (value) => value.replace(/\s+/g, ' ').trim();
const toExactWordCount = (text, targetWords) => {
    const words = normalizeText(text).split(/\s+/).filter(Boolean);
    return words.slice(0, targetWords).join(' ');
};
const toExactWordCountFromSources = (primary, fallbackSources, targetWords) => {
...
const buildOptionSet = (correct, distractors, seed) => {
    const deduped = Array.from(new Set(distractors
...
const getQuestionKey = (question) => normalizeText(question).toLowerCase();
const reserveQuestionText = (question) => {
    const key = getQuestionKey(question);
    if (seenQuestionTexts.has(key))
        return false;
    seenQuestionTexts.add(key);
    if (seenQuestionTexts.size > QUESTION_CACHE_LIMIT) {
        const oldest = seenQuestionTexts.values().next().value;
        if (oldest)
            seenQuestionTexts.delete(oldest);
    }
    return true;
};
const reservePredictionText = (question) => {
    const key = getQuestionKey(question);
    if (seenPredictionTexts.has(key))
        return false;
    seenPredictionTexts.add(key);
    if (seenPredictionTexts.size > PREDICTION_CACHE_LIMIT) {
        const oldest = seenPredictionTexts.values().next().value;
        if (oldest)
            seenPredictionTexts.delete(oldest);
    }
    return true;
};
const buildContentPrediction = (article) => {
...
const buildFastSummary = (article) => {
    ...
    return toExactWordCountFromSources(baseSummary, [content, summary, headline, source, publishedAt], SUMMARY_WORD_TARGET);
};
const buildQuizVariants = (article) => {
    const topic = pickTopic(article);
    const terms = extractKeyTerms(`${article.headline} ${article.summary} ${article.fullContent}`);
    ...
    const q1 = buildOptionSet(`${first} is the hinge detail that changes how the headline should be interpreted`, [
    ...
    const q2 = buildOptionSet(`The article implies a chain where ${second} influences outcomes through ${third}`, [
    ...
    const q3 = buildOptionSet(`If the evidence around ${numberHint} is revised, the conclusion about ${first} weakens first`, [
    ...
    const q4 = buildOptionSet(`A direct contradiction would show ${fifth} moving opposite to the article's projected consequence`, [
    ...
    const q5 = buildOptionSet(`The most exposed stakeholder in this framing is the group tied to ${fourth}`, [
    ...
    const q6 = buildOptionSet(`In timeline terms, the article places ${publishedAt} as the point after which ${third} becomes actionable`, [
...
export const buildFastArticleContent = (article) => {
...
const normaliseGeneratedContent = (article, parsed) => {
    ...
    return {
        ...
        summary: (() => {
            ...
            return toExactWordCountFromSources(candidate, [article.summary, article.fullContent, article.headline, article.source, article.publishedAt], SUMMARY_WORD_TARGET);
        })(),
        quiz: normalizedQuiz,
        prediction: (() => {
...
const parseGeneratedContent = (article, raw) => {
...
const buildCacheKey = (article) => [article.headline, article.summary, article.fullContent, article.category, article.source, article.publishedAt]
    .join('||')
    .toLowerCase();
const rememberGeneratedContent = (key, content) => {
    generatedContentCache.set(key, content);
    if (generatedContentCache.size > MAX_CACHE_ENTRIES) {
        const oldestKey = generatedContentCache.keys().next().value;
        if (oldestKey) {
            generatedContentCache.delete(oldestKey);
        }
    }
};
const runQueuedGeneration = async (task) => {
    let release;
    const current = generationQueue.then(() => new Promise((resolve) => {
        release = resolve;
    }));
