...
const generateCauseChainPrompt = (headline, summary, category) => {
...
const parseCauseChainResponse = (response) => {
    try {
        ...
        const edges = parsed.edges
            ...
            .filter((edge) => edge.from && edge.to && nodes.some(n => n.id === edge.from) && nodes.some(n => n.id === edge.to));
        if (edges.length === 0) {
            return null;
        }
        return {
...
const generateMockCauseChain = (headline, summary) => {
    const nodes = [
...
export const generateCauseChain = async (headline, summary, category) => {
    if (!hasBytezKey()) {
...
export const generateFeedbackPrompt = (question, nodes, edges, userChain, userConnections) => {
...
export const generateCauseChainFeedback = async (question, nodes, edges, userChain, userConnections) => {
...
export const validateCauseChain = (userConnections, correctEdges, userChain, nodes) => {
    ...
    for (const userConn of userConnections) {
        const isCorrect = correctEdges.some(edge => edge.from === userConn.from && edge.to === userConn.to);
        if (isCorrect) {
            correctConnections++;
        }
    }
...
export const calculateXPReward = (validation) => {
    let xpEarned = 0;
    let xpPenalty = 0;
